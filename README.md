...
| 牛客校招日程 | 纯 HTTP | 校招日程聚合页 SSR（22000+公司库，每日最新更新的20家：网申起止/城市/官网直链） |
| BOSS直聘 | 本地 CLI | 基于 boss-agent-cli（需本机 `boss login` 扫码），按"校招/应届/届"过滤社招噪声；未登录自动跳过 |
...
**聚合平台探测结论（都有墙，暂不可直连）**：猎聘（api-c 签名校验 -1400）、智联（接口路径已变，boss-agent-cli 内实现也需登录）、实习僧（SSR 为混淆 NUXT 函数）、前程无忧51job（阿里云 WAF）、国聘 iguopin（gp-api 需登录 403 / appv5 加密）。

**抓不到直连、靠聚合源（牛客/应届生/牛客日程）覆盖的公司**：华为（新版SPA岗位接口未公开）、OPPO（职位接口藏在项目交互后，但牛客日程有其网申直链）、海康威视（岗位接口需登录）、比亚迪（需登录token）、中兴/大疆（挂MOKA平台且响应加密，牛客日程有 MOKA 直链）、荣耀（TLS异常）、宁德时代（域名未开放）、米哈游（ATS需渠道参数）、宇树等。后续可按 `scraper/` 下任意适配器的模式扩展。
